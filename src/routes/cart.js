...
const { db, getOrCreateCartIdForUser, touchUpdatedAt } = require('../db');
...
router.post(
  ...
  (req, res) => {
    ...
    if (existing) {
      ...
      touchUpdatedAt('cart_items', existing.id);
    } else {
      db.prepare('INSERT INTO cart_items (cart_id, product_id, quantity) VALUES (?, ?, ?)').run(cartId, product_id, qty);
    }
    touchUpdatedAt('carts', cartId);
    res.redirect('/cart');
  }
);

router.post('/cart/item/:id/update', requireAuth, body('quantity').isInt({ min: 1 }), (req, res) => {
  ...
  touchUpdatedAt('cart_items', ci.id);
  res.redirect('/cart');
});

router.post('/cart/item/:id/delete', requireAuth, (req, res) => {
  const ci = db.prepare('SELECT * FROM cart_items WHERE id = ?').get(req.params.id);
